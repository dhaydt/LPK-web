          ...
          <template v-for="item in menuItems">
            ...
            >
              ...
              <router-link
                :to="item.link"
                v-if="!hasItems(item)"
                :class="`side-nav-link-ref ` + item.role"
              >
                <i :class="`bx ${item.icon}`" v-if="item.icon"></i>
                <span>{{ item.label }}</span>
                <span
              ...
              <ul v-if="hasItems(item)" class="sub-menu" aria-expanded="false">
                <li v-for="(subitem, index) of item.subItems" :key="index">
                  <router-link
                    ...
                    :class="`side-nav-link-ref ` + subitem.role"
...
export default {
  ...
  mounted: function() {
    ...
    var show = document.getElementsByClassName(this.auth);
    // console.log("menu", document.getElementsByClassName(this.auth));
    for (var s = 0; s < show.length; s++) {
      // console.log("s", show[s]);
      show[s].classList.add("d-none");
    }
